...
- [ ] 사용자는 메모, 초안, 링크를 아무렇게나 넣어도 글 재료로 정리된 결과를 볼 수 있다
- [ ] 시스템은 글 작성 전에 핵심 메시지와 글의 방향을 먼저 제안한다
- [ ] 시스템은 정보가 비었을 때만 짧고 정확한 보완 질문을 한다
- [ ] 사용자는 LinkedIn 글과 블로그 글 중 결과 형식을 선택할 수 있다
- [ ] 결과물은 항상 출처를 남기고 사람처럼 자연스럽게 작성된다
- [ ] 시스템은 사용자 프로필과 수정 습관을 기억해 다음 결과에 반영한다
...
|----------|-----------|---------|
| 제품의 중심 가치를 "요약"이 아니라 "재구성"으로 둔다 | 사용자가 원하는 것은 정보 압축이 아니라 자기 생각이 살아 있는 결과물이다 | — Pending |
| v1부터 가벼운 개인화를 포함한다 | 이 제품의 차별점은 사용자다움이며, 이를 뒤로 미루면 일반 글쓰기 도구와 차별이 약해진다 | — Pending |
| 결과물에는 항상 출처를 남긴다 | 링크와 외부 자료 기반 글쓰기는 신뢰와 추적 가능성이 중요하다 | — Pending |
| 생성 결과는 사람처럼 자연스럽게 쓰고, 이모티콘과 마크다운 강조를 금지한다 | 사용자는 AI 티 나는 결과를 원하지 않는다 | — Pending |
| 질문 수는 최소화하고 필요한 경우에만 2-4개 수준으로 제한한다 | 질문이 많아지면 글쓰기 도구가 아니라 설문 흐름이 된다 | — Pending |
